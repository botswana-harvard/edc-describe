...
class ModelInstanceCounter(object):
    ...
    def _set(self):
        ...
        for app_label in app_labels:
            ...
            self. model_list[app_label] = [
                {'verbose_name': model._meta.verbose_name, 'object_name': model._meta.object_name, 'model': model}
                for model in models.get_models()
                if (model._meta.app_label in app_label) and key_field in [
                    field.name for field in model._meta.fields] and '_audit_id' not in [
                        field.name for field in model._meta.fields]]
        for app_label, result in self.model_list.iteritems():
            counts = []
            for r in result:
                counts.append(
                    [r['verbose_name'], r['object_name'].lower(),
                     r['model'].objects.filter(hostname_created=self.producer).count()])
            counts.sort()
